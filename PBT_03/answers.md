...
- Không cần file ngoài. Nhược điểm:
- Khó bảo trì khi nhiều element.
- Không tái sử dụng được.
- Độ ưu tiên cao nhất, dễ gây override không mong muốn. Khi nào dùng:
...
- Dễ dùng cho prototype hoặc trang đơn giản. Nhược điểm:
- Không tái sử dụng cho nhiều trang.
- File HTML to hơn. Khi nào dùng:
...
- Rule D p.price { color: green; }: specificity: 0,1,1
  Màu cuối cùng của element: <p class="price" id="main-price">: red. Lý do: rule C có specificity cao nhất (ID selector) nên thắng.
...
- p { color: orange; }: 0,0,1
- body p { color: red; }: 0,0,2
- .text { color: yellow; }: 0,1,0
- .highlight { color: green; }: 0,1,0
- p.text { color: blue; }: 0,1,1
- .text.highlight { color: violet; }: 0,2,0
- p.text.highlight { color: pink; }: 0,2,1
- #demo { color: purple; }: 1,0,0
- p#demo { color: brown; }: 1,0,1
- #demo.text { color: aqua; }: 1,1,0

2. Element cuối cùng hiển thị màu aqua (rule số 10) vì rule số 10 có specificity cao nhất (1,1,0): 1 id + 1 class + 0 element
3. Thay đổi thứ tự rules trong file kết quả không đổi vì specificity quyết định, không phải thứ tự. Rule có specificity cao hơn luôn thắng, bất kể vị trí trong CSS file.
